...
module.exports = {
	...
	viewExportIngredient: function(req, res) {
		if(!req.session.user) {
	    	...
	    	var query = "SELECT DISTINCT i.`store`,s.`name` FROM `ingredientstore` i JOIN `store` s ON s.`id` = i.`store` where s.deleted = 0 AND i.deleted = 0 AND i.store != 1 AND i.`instock` <= i.`limit` ORDER BY i.store";
